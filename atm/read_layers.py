...
def read_layers(self):
    ...
    if self.control['Read_Geotiff'].lower() == 'yes':
        ...
        """ Read Data Layers """
        #~ initial_cohort_list = self.control['Initial_Cohort_List']
        #~ if self.control['Simulation_area'].lower() == 'barrow':
            #~ initial_cohort_list = str(self.control['Run_dir']+self.control['Input_dir']+'/Barrow/'+\
                                          #~ self.control['Control_dir']+'/'+self.control['Initial_Cohort_List'])
        #~ elif self.control['Simulation_area'].lower() == 'yukon':
            #~ initial_cohort_list = str(self.control['Run_dir']+self.control['Input_dir']+'/Yukon/'+\
                                          #~ self.control['Control_dir']+'/'+self.control['Initial_Cohort_List'])
        #~ elif self.control['Simulation_area'].lower() == 'tanana':
            #~ initial_cohort_list = str(self.control['Run_dir']+self.control['Input_dir']+'/Tanana/'+\
                                          #~ self.control['Control_dir']+'/'+self.control['Initial_Cohort_List'])

        #~ with open(initial_cohort_list, 'r') as geotiff_filenames:
#        with open(self.control['Initial_Cohort_List']t, 'r') as geotiff_filenames:
        for filename in self.control['Initial_Cohort_List']:
