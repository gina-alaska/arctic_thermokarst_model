def read_alem_control(self):
    ...
    Evolution Model (ALEM) Control file. This control file is needed to initialize
    the ALEM with information regarding Input and Output directories, model
    resolution, cohort names, etc.

    The name of the control file is specified at the execution of the model
    as the first option following the program name. See example below.
    ------------------------------
    $ python ALEM.py ALEM_Control
    ------------------------------
    In this case, the file named "ALEM_Control" is the control file and is
    located in the model directory.
    
    ###############################################
      NTB! Add more information as needed/required
    ###############################################
    """
    print '    Reading the ALEM control file: ', self.Control_file
    self.control = {}
    ...
    # Run, Input, Output, Archive, Control & Notes Directories
    self.Run_Directory     = self.control['Run_directory']
    self.Input_Directory   = self.control['Input_directory']
    self.Output_Directory  = self.control['Output_directory']
    self.Control_Directory = self.control['Control_directory']
    self.Notes_Directory   = self.control['Notes_directory']
    #_____________________________________________
    # Model Domain Information
    self.LL_X_coordinate    = self.control['LL_X_coordinate']
    self.LL_Y_coordinate    = self.control['LL_Y_coordinate']
    self.UR_X_coordinate    = self.control['UR_X_coordinate']
    self.UR_Y_coordinate    = self.control['UR_Y_coordinate']
    self.X_model_resolution = self.control['X_model_resolution']
    self.Y_model_resolution = self.control['Y_model_resolution']

    #=======================================================
    # Convert string variables to numbers if necessary
    #=======================================================
    self.LL_X_coordinate    = float(self.LL_X_coordinate)
    self.LL_Y_coordinate    = float(self.LL_Y_coordinate)
    self.UR_X_coordinate    = float(self.UR_X_coordinate)
    self.UR_Y_coordinate    = float(self.UR_Y_coordinate)
    self.X_model_resolution = float(self.X_model_resolution)
    self.Y_model_resolution = float(self.Y_model_resolution)

    #=======================================================
    # Control file error check
    #=======================================================
    if self.LL_X_coordinate >= self.UR_X_coordinate:
        print ' '
        print '-------------------------------------------------------------------'
        print 'ERROR [Control file]:' 
        print 'Lower left X-coordinate ('+ str(self.LL_X_coordinate)+') must be less than the'
        print 'Upper right X-coordinate ('+str(self.UR_X_coordinate)+').'
        print 'ENDING SIMULATION.'
        print '-------------------------------------------------------------------\n'
        exit()
    if self.LL_Y_coordinate >= self.UR_Y_coordinate:
        print ' '
        print '-------------------------------------------------------------------'
        print 'ERROR [Control file]:' 
        print 'Lower left Y-coordinate ('+ str(self.LL_Y_coordinate)+') must be less than the'
        print 'Upper right Y-coordinate ('+str(self.UR_Y_coordinate)+').'
        print 'ENDING SIMULATION.'
        print '-------------------------------------------------------------------\n'
        exit()        
    ...
    #=======================================================
    # Quick calculation area if needed
    #=======================================================
    # Number cols (x-direction), rows (y-direction)
    self.ncols = int((self.UR_X_coordinate - self.LL_X_coordinate)/self.X_model_resolution)
    self.nrows = int((self.UR_Y_coordinate - self.LL_Y_coordinate)/self.Y_model_resolution)

    #print 'The number of rows and columns are: ', self.ncols, self.nrows
    #--------------------------------------------------------
